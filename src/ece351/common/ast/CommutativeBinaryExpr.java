...
public abstract class CommutativeBinaryExpr extends BinaryExpr {
	...
	private boolean examine(final Examiner e, final Examinable obj) {
		...
		// fixed errors with conditions:
		// **************************************** NEW CODE ****************************************
		if(e.examine(left, cbe.left) && e.examine(right, cbe.right)){
			return true;
		}else if(e.examine(left, cbe.left) && e.examine(left, cbe.right) && e.examine(right, cbe.left)){
			return true;
		}else if(e.examine(left, cbe.right) && e.examine(right, cbe.left)){
			return true;
		}else{
			return false;
		}
